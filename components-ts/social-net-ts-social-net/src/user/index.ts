...
export function connectUserAgent(
  ...
  let existingConnection =
    existingIdx !== -1 ? user.connectedUsers[existingIdx]![1] : undefined;
...
export function disconnectUserAgent(
  ...
  if (shouldDisconnect) {
    if (existingConnection!.connectionTypes.length === 1) {
      ...
      existingConnection!.connectionTypes =
        existingConnection!.connectionTypes.filter((c) => c !== connectionType);
      existingConnection!.updatedAt = now;
    }
    user.updatedAt = now;
    return true;
...
export function userMatchesQuery(user: User, query: Query): boolean {
  for (const [field, value] of query.fieldFilters) {
    ...
    switch (field.toLowerCase()) {
      ...
      case "connected-users":
      case "connectedusers":
        matches = user.connectedUsers.some((u) =>
          textExactMatches(u[0], value),
        );
        break;
      default:
        matches = false;
    ...
    if (userIds.length > 0) {
      ...
      for (const ids of idsChunks) {
        ...
        const promises = ids.map(
          async (id) => await UserAgent.get(id).getUserIfMatch(parsedQuery),
        );
