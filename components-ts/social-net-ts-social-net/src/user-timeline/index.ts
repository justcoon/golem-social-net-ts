...
export class UserTimelineAgent extends BaseAgent {
  ...
  ): Promise<UserTimelineUpdates | null> {
    if (this.state !== null) {
      ...
      const updates = this.state.posts.filter(
        (p) => p.updatedAt.timestamp > since.timestamp,
      );
      return {
  ...
  @description(
    "Triggered when posts are updated to be added/modified on the timeline",
  )
...
export class UserTimelineViewAgent extends BaseAgent {
  ...
  async getPostsUpdatesView(
    ...
    const timelineUpdates =
      await UserTimelineAgent.get(userId).getUpdates(updatesSince);
