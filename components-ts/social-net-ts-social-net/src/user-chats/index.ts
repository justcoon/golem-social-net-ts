...
export class UserChatsAgent extends BaseAgent {
  ...
  async getUpdates(updatesSince: Timestamp): Promise<UserChatsUpdates | null> {
    if (this.state !== null) {
      ...
      const updates = this.state.chats.filter(
        (c) => c.updatedAt.timestamp > since.timestamp,
      );
      return {
...
async function fetchChatsByIds(
  ...
  for (const chunk of chunks) {
    const promises = chunk.map((id) =>
      query
        ? ChatAgent.get(id).getChatIfMatch(query)
        : ChatAgent.get(id).getChat(),
    );
...
export class UserChatsViewAgent extends BaseAgent {
  ...
  async getChatsUpdatesView(
    ...
    const userChatsUpdates =
      await UserChatsAgent.get(userId).getUpdates(updatesSince);
